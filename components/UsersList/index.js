...
class UsersList extends Component {
  ...
  fetchingSuccessSenario = () => {
    console.log("fetching is success");
  };

  renderLoader = () => (
    <div>
      <p>Your Network is too slow to loade content</p>
    </div>
  );

  render() {
    const { isClick, page, isLoading } = this.state;
    console.log(page);
    if (isClick === true) {
      this.getUsersList();
    }
    ...
    return (
      <div>{isLoading ? this.renderLoader() : this.displayUsersList()}</div>
    );
  }
}
